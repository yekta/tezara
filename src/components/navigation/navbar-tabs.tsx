...
export default function NavbarTabs({ className, classNameTab }: Props) {
  ...
  const universitiesPath = useAtomValue(previousPathForUniversitiesPageAtom);
  ...
  const tabs: {
    ...
    return [
      { label: "Ara", pathname: "/search", href: "/search", Icon: SearchIcon },
      {
        ...
        href: universitiesPath ? universitiesPath : universitiesRoute,
        Icon: LandmarkIcon,
      },
    ];
  }, [universitiesPath]);
      ...
      {tabs.map((tab) => (
        <LinkButton
          ...
          className={cn(
            ...
            shrink min-w-0 data-[active]:bg-border/60 rounded-lg h-full flex items-center justify-center 
